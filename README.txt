...
The submission bundle should include the following:
   - This README.txt file
   - hl_enc_v101.py Python program
   - The original userdata.txt file
   - The encounter_output.txt output generated by the program on userdata.txt

The hl_enc_v101.py program file holds all the major documentation for the
problem statement, givens, assumptions, main algorithm, and a few ideas
for optimization.

To run the program:
  On any Linux/UNIX or MacOS machine with Python installed, simply execute
  the following at the user prompt with the userdata.txt available in the
  same directory.
    ./hl_enc_v101.py

  The program can take a variety of arguments as well, displayed with -h:
    ./hl_enc_v101.py -h

  For use with a different input file or location for userdata.txt, just
  execute the program with a file path name:
    ./hl_enc_v101.py inputFiles/sample_input2.txt
...
where name1 is lexicographically lower than name2 and lines are sorted in
